...
<ul class="columns-2 checkout-form clearfix" id="form1">
    <li class="col2 clearfix">
        <div class="tag-title-wrap clearfix"><h4 class="tag-title">Artists</h4></div>
<table width="400" border="2" >
    <tr>
        <th width="100">Genric:  </th>
...
try{
    ...
    $stmt1 = $conn->prepare("SELECT * FROM Music_Category");
    $stmt1->execute();

    while($row1 = $stmt1->fetch()){

        $stmt2 = $conn->prepare("SELECT * FROM Artist_Feature, Artists, User_Login
        where User_Login_Id = Artist_Id and Artist_Id = Musician_Id and Feature_Id = ? ");

        $stmt2->bindParam(1, $row1[Category_id]);
        $stmt2->execute();
...
?>
    <tr>
        <th> <?= $row1[Category_Name] ?> </th>
        <th><a href="Artists_personal.php?user_id=<?= $row2[User_Login_Id] ?>"><?= $row2[User_Login_Name] ?></a></th>
